...
//      TODO (8) Display the summary that you created above
        ...
        String date = SunshineDateUtils.getFriendlyDateString(mContext, mCursor.getLong(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE)), false);
        int descriptionCol = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
        int lowCol = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);
        int highCol = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, descriptionCol);
        double low = mCursor.getDouble(lowCol);
        double high = mCursor.getDouble(highCol);
        String highAndLowTemp = SunshineWeatherUtils.formatHighLows(mContext, high, low);
        String weatherSummary = date + " - " + description + " - " + highAndLowTemp;
